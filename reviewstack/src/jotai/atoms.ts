...
import type {DevstackPullRequestMetadata, StackLayer} from '../devstackStack';
 ...
 */
function dropAllDatabases(indexedDB: IDBFactory): Promise<unknown> {
  // localhost:3000 is commonly shared by unrelated development applications.
  // indexedDB.databases() returns every database for the origin, so deleting
  // that list can block on (and destroy) data owned by another application.
  // ReviewStack already maintains the authoritative list of database names it
  // has used; restrict cleanup to that list on every browser.
  const databaseNames = [...ALL_DB_NAMES_EVER];
  ...
  return Promise.all(
    databaseNames.map(name => {
      return new Promise((resolve, reject) => {
        ...
        request.onblocked = () => {
          // Keep waiting: onsuccess is the browser's acknowledgement that all
          // connections have handled versionchange and released this database.
        };
...
export const gitHubTokenPersistenceAtom = atom(
  ...
  (get, set, token: string | null) => {
    // Any credential change clears the cache, so every existing database
    // connection must close regardless of whether this is login or logout.
...
export const gitHubPullRequestVersionDiffAtom = atom<Promise<DiffWithCommitIDs | null>>(
  async get => {
    ...
    const reviewTarget = get(gitHubPullRequestReviewTargetAtom);
    if (reviewTarget.type === 'commit') {
      const versions = await get(gitHubPullRequestVersionsAtom);
      const selectedCommit = versions
        .flatMap(version => version.commits)
        .find(commit => commit.commit === reviewTarget.commitID);
      const firstParent = selectedCommit?.parents[0];
      if (selectedCommit == null || firstParent == null) {
        return null;
      }
      return get(
        gitHubDiffForCommitsAtom({baseCommitID: firstParent, commitID: selectedCommit.commit}),
      );
    }
    ...
    if (beforeCommitID != null) {
      // Get the merge base for each explicit version comparison.
      ...
      // Version construction is the authority for the base of an ordinary
      // layer diff. In particular, Sapling/devstack metadata sets this to the
      // lower PR's head even when a cross-fork GitHub PR targets main.
      const versions = await get(gitHubPullRequestVersionsAtom);
      const versionBaseCommitID = baseParentForVersion(versions, afterCommitID);
      if (versionBaseCommitID !== undefined) {
        if (versionBaseCommitID == null) {
          return null;
        }
        return get(
          gitHubDiffForCommitsAtom({baseCommitID: versionBaseCommitID, commitID: afterCommitID}),
        );
      }

      // Preserve the legacy fallback for commits that are not represented by
      // a version (for example while timeline data is still converging).
      const afterBaseParent = await get(gitHubPullRequestCommitBaseParentAtom(afterCommitID));
      const afterBaseCommitID = afterBaseParent?.oid;
...
/** Change counts for the exact texts and commits shown by the current diff. */
export const gitHubPullRequestVersionDiffStatsAtom = atom<
  Promise<
...
>(async (get, {signal}) => {
  ...
  const isSubmoduleChange = (change: CommitChange) =>
    change.type === 'modify'
  ...
  const modeChanges = diff.diff.filter(
    change =>
      change.type === 'modify' &&
      change.before.oid === change.after.oid &&
      change.before.mode !== change.after.mode,
  );
  const submoduleChanges = diff.diff.filter(isSubmoduleChange).length;
  return {
    ...stats,
...
export const gitHubPullRequestVersionsAtom = atom<Promise<Version[]>>(async get => {
  ...
  // Stacked layers may contain more than one commit.
  const stackedPR = get(stackedPullRequestAtom);
  if (stackedPR.type === 'sapling' || stackedPR.type === 'devstack') {
    const stack = get(stackedPullRequestMetadataAtom);
    if (stack == null) {
      return [];
    }
    const fragments = await get(stackedPullRequestFragmentsAtom);
    if (fragments.length !== stack.stack.length) {
      const expected = stack.stack.map(layer => `#${layer.number}`).join(', ');
      const received = fragments.map(fragment => `#${fragment.number}`).join(', ') || 'none';
      throw new StackMetadataError(
        `Could not load the complete stack. Expected ${expected}; received ${received}. ` +
          'Check that every PR exists and is accessible to the current GitHub token.',
      );
    }
    versions.reverse();

    const index = stack.currentStackEntry;
    const parentFragment = fragments[index + 1];

    const currentLayer = stack.stack[stack.currentStackEntry];
    ...
    const allFetchedCommits = await Promise.all(
      commitOidsToFetch.map(oid => get(gitHubCommitAtom(oid))),
    );
    const commitsByOid = new Map<GitObjectID, Commit>();
    allFetchedCommits.forEach((commit, idx) => {
      if (commit != null) {
        commitsByOid.set(commitOidsToFetch[idx], commit);
      }
    });
    ...
    for (const version of versions) {
      ...
      const isLatestVersion = version.oid === latestCommit?.oid;
      // Version 2 metadata records the exact commits owned by this layer.
      // Use that order as authoritative for the latest version rather than
      // inferring membership from a cumulative PR timeline.
      let validCommits: Commit[];
      if (isLatestVersion && currentLayer.commits != null) {
        validCommits = exactCommitsForLayer(currentLayer.commits, commitsByOid);
      } else {
        validCommits = commitFragmentsForPRVersion
          .map(c => commitsByOid.get(c.oid))
          .filter(notEmpty);
      }

      const versionCommits: VersionCommit[] = [];
      // Force-push snapshots describe historical versions; they are not real
      // commits in the current layer and must not appear in Commit mode.
      if (!isLatestVersion || currentLayer.commits == null) {
...
export type StackedPullRequest =
  ...
  | {
      type: 'devstack';
      metadata: DevstackPullRequestMetadata;
    }
  | {
      type: 'invalid-stack';
      provider: 'devstack';
...
export const stackedPullRequestAtom = atom<StackedPullRequest>(get => {
  ...
  if (body != null) {
    const devstack = parseDevstackStackBodyResult(body);
    if (devstack.type === 'valid') {
      return {type: 'devstack', metadata: devstack.metadata};
    } else if (devstack.type === 'invalid') {
      return {type: 'invalid-stack', provider: 'devstack', message: devstack.message};
    }
...
type PullRequestStackMetadata = {
  stack: StackLayer[];
  currentStackEntry: number;
};

/** Normalized layer data shared by stack providers. */
const stackedPullRequestMetadataAtom = atom<PullRequestStackMetadata | null>(get => {
  const stacked = get(stackedPullRequestAtom);
  switch (stacked.type) {
    case 'sapling':
      return {
        stack: stacked.body.stack,
        currentStackEntry: stacked.body.currentStackEntry,
      };
    case 'devstack':
...
const stackedPullRequestNumbersAtom = atom<number[]>(get => {
  const stacked = get(stackedPullRequestAtom);
  const metadata = get(stackedPullRequestMetadataAtom);
  if (metadata != null) {
    return metadata.stack.map(({number}) => number);
  }
  switch (stacked.type) {
    case 'no-stack':
      return [];
