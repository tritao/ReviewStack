...
export function openDatabase(): Promise<IDBDatabase> {
  return _openDatabase().then(db => {
    // deleteDatabase() sends versionchange to every open connection. Closing
    // here makes database deletion a reliable cross-tab/worker handshake even
    // if a context misses or delays the logout BroadcastChannel message.
